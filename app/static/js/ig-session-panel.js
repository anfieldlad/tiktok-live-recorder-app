function initIgSessionPanel() {
  ...
  async function refreshSessionStatus() {
    const [cookieResponse, loginResponse] = await Promise.all([fetch(appPath("/instagram/auth/status")), fetch(appPath("/instagram/auth/login-browser/status"))]);
    ...
    if (cookieBody.configured) setNotice(sessionNotice, "Your Instagram session is ready.", "success");
  ...
  async function saveCookies(event) {
    ...
    try {
      const response = await fetch(appPath("/instagram/auth/cookies"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionid: sessionValue }) });
  ...
  async function importCookies(browserName) {
    try {
      ...
      const response = await fetch(appPath(`/instagram/auth/import-browser/${browserName}`), { method: "POST" });
  ...
  async function startBrowserLogin(browserName) {
    try {
      ...
      const response = await fetch(appPath(`/instagram/auth/login-browser/${browserName}/start`), { method: "POST" });
  ...
  async function captureBrowserLogin() {
    try {
      ...
      const response = await fetch(appPath("/instagram/auth/login-browser/capture"), { method: "POST" });
  ...
  async function closeBrowserLogin() {
    try {
      const response = await fetch(appPath("/instagram/auth/login-browser/close"), { method: "POST" });
