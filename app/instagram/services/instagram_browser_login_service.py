...
class InstagramBrowserLoginService:
    def __init__(self, project_root: Path, cookie_service: InstagramCookieService) -> None:
        self.cookie_service = cookie_service
        self.session_root = project_root / "data" / "instagram-browser-login"
    ...
    def capture_session(self) -> dict:
        ...
        if "sessionid" not in cookies:
            raise ValueError("sessionid was not found yet. Please finish logging in on Instagram and close the login browser first")
    ...
    def _resolve_browser_path(self, browser_name: str) -> Path:
        if os.name != "nt":
            raise ValueError(
                ...
                "On this server, save sessionid manually or import cookies another way."
            )
        browser_paths = {
            "chrome": [
